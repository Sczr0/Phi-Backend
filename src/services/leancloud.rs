...
impl LeanCloudService {
    pub fn new() -> Self {
        ...
        phi.append("X-LC-Key", "Qr9AEqtuoSVS3zeD6iVbM4ZC0AtkJcQ89tywVyi0".parse().unwrap());
    ...
    pub async fn login_with_taptap(&self, token: &TapTapToken, openid: &str, unionid: &str) -> Result<Value> {
        let body = format!(
        ...
        let response = self.client.post("https://rak3ffdi.cloud.tds1.tapapis.cn/1.1/users")
