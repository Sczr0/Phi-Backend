...
pub async fn resolve_token(
    ...
    if let (Some(platform), Some(platform_id)) = (&req.platform, &req.platform_id) {
        if !platform.trim().is_empty() && !platform_id.trim().is_empty() {
            log::debug!("尝试通过请求体平台 '{}' 的 ID '{}' 查询 Token", platform, platform_id);
            match user_service.get_binding_by_platform_id(platform, platform_id).await {
                Ok(binding) => {
                    log::debug!("通过平台ID查询到绑定，获取 Token: {}", binding.session_token);
                    return Ok(binding.session_token);
                }
                Err(AppError::UserBindingNotFound(_)) => {
                    log::warn!("平台 '{}' 的 ID '{}' 未找到或未绑定 Token", platform, platform_id);
                    return Err(AppError::UserBindingNotFound(
                        format!("平台 {} 的 ID {} 未找到或未绑定", platform, platform_id)
                    ));
                }
                Err(e) => {
    ...
    Err(AppError::Other("请在请求中提供 'token' 或 'platform'+'platform_id' 字段".to_string()))
}
    ...
    let token = match &req.token {
        ...
        _ => {
            if let (Some(platform), Some(platform_id)) = (&req.platform, &req.platform_id) {
                ...
                return Err(AppError::BadRequest("请在请求中提供 'token' 或 'platform'+'platform_id' 字段".to_string()));
            }
        }
    };
