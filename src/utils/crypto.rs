...
pub fn encrypt(data: &[u8]) -> AppResult<Vec<u8>> {
    ...
    if key.len() != 32 {
        return Err(AppError::AesError(format!(
            "AES密钥长度不正确，期望32字节，实际{}字节",
            key.len()
        )));
    }
    ...
    let cipher =
        Aes256CbcEnc::new_from_slices(&key, &iv).map_err(|e| AppError::AesError(e.to_string()))?;
...
pub fn decrypt(data: &[u8]) -> AppResult<Vec<u8>> {
    ...
    let cipher =
        Aes256CbcDec::new_from_slices(&key, &iv).map_err(|e| AppError::AesError(e.to_string()))?;
