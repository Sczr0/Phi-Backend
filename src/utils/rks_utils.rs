...
pub fn calculate_player_rks_details(records: &[RksRecord]) -> (f64, f64) {
    ...
    // 计算 Best 27 RKS 总和
    let best_27_sum: f64 = sorted_records
        .iter()
    ...
    let ap_records: Vec<&RksRecord> = sorted_records.iter().filter(|r| r.acc >= 100.0).collect(); // 这里已经是按 RKS 降序的，因为是从 sorted_records 筛选的

    // 计算 AP Top 3 RKS 总和
    let ap_top_3_sum: f64 = ap_records
        .iter()
    ...
    log::debug!(
        "[B30 RKS] AP Top 3 计算: AP成绩数={}, 使用了 {} 个，总和 = {:.4}",
        ap_records.len(),
        ap3_count,
        ap_top_3_sum
    );
    ...
    log::debug!("[B30 RKS] 最终 RKS 计算: (B27 Sum {best_27_sum:.4} + AP3 Sum {ap_top_3_sum:.4}) / 30 = {final_exact_rks:.6} -> Rounded {final_rounded_rks:.2}");
...
pub fn calculate_chart_rks(acc_percent: f64, constant: f64) -> f64 {
    ...
    log::debug!(
        "谱面RKS计算: ACC={acc_percent:.2}%, 定数={constant:.1}, ACC={acc_factor:.4}, RKS={rks:.4}"
    );
...
impl PrecalculatedRksDetails {
    fn from_sorted_records(records: &[RksRecord]) -> Self {
        ...
        let ap_records_sorted_by_rks: Vec<RksRecord> =
            records.iter().filter(|r| r.acc >= 100.0).cloned().collect(); // 已经是按RKS排序的
...
fn simulate_rks_increase(
    ...
    log::debug!("模拟RKS增长: 目标谱面={target_chart_id_full}, 测试ACC={test_acc:.4}%, 定数={target_chart_constant:.1}");

    // 分离song_id和difficulty
    let parts: Vec<&str> = target_chart_id_full.rsplitn(2, '-').collect();
    if parts.len() != 2 {
        return precalculated.exact_rks;
    } // 格式错误则返回原值
    let song_id = parts[1];
    ...
    // 查找原记录信息
    let original_record_opt = all_sorted_records
        .iter()
        .find(|r| r.song_id == song_id && r.difficulty == difficulty);

    let original_is_ap = original_record_opt.is_some_and(|r| r.acc >= 100.0);
    ...
    if let Some(original_record) = original_record_opt {
        if let Some(pos) = b27_candidates.iter().position(|r| {
            r.song_id == original_record.song_id && r.difficulty == original_record.difficulty
        }) {
    ...
    let _b27_min_rks = if b27_candidates.len() < 27 && !removed_from_b27 {
        ...
        if b27_candidates.len() >= 27 {
            // B27已满
            min_rks_in_current_b27
        } else {
            // B27未满 (因为移除了一个)，需要考虑第28个记录
            all_sorted_records
                .get(27)
                .map_or(min_rks_in_current_b27, |next_r| {
                    next_r.rks.min(min_rks_in_current_b27)
                })
        }
    };
    ...
    if simulated_chart_rks > b27_threshold || removed_from_b27 {
        ...
        log::trace!(" + 添加到B27 Sum: {target_chart_id_full} (RKS {simulated_chart_rks:.4})");
        ...
        if !removed_from_b27 && precalculated.b27_records.len() >= 27 {
            if let Some(record_to_remove) = precalculated.b27_records.last() {
                ...
                log::trace!(
                    " - B27中被挤掉的记录: {} (RKS {:.4})",
                    record_to_remove.song_id,
                    record_to_remove.rks
                );
            }
        }
    ...
    // 移除旧AP记录（如果存在）
    if original_is_ap {
        if let Some(pos) = ap_candidates
            .iter()
            .position(|r| r.song_id == song_id && r.difficulty == difficulty)
        {
    ...
    // 添加新AP记录（如果模拟结果是AP）
    if simulated_is_ap {
        let new_ap_record = RksRecord {
            // 构造一个临时的 RksRecord 用于排序
        ...
        log::trace!(" + 添加到AP列表: {target_chart_id_full} (RKS {simulated_chart_rks:.4})");
    }

    // 重新计算AP3 Sum
    let new_ap3_sum = ap_candidates.iter().take(3).map(|r| r.rks).sum();
    log::trace!(" - 旧AP3 Sum: {current_ap3_sum:.4}, 新AP3 Sum: {new_ap3_sum:.4}");
    ...
    log::debug!(
        "模拟RKS增长结果: 原RKS={:.6}, 模拟RKS={:.6}",
        precalculated.exact_rks,
        simulated_exact_rks
    );
...
pub fn calculate_target_chart_push_acc(
    ...
    log::debug!("当前玩家RKS: 精确值={current_exact_rks:.6}, 四舍五入={current_rounded_rks:.2}");
    ...
    let target_rks_threshold = {
        ...
        log::debug!("目标RKS阈值计算: 当前RKS={current_exact_rks:.6}, 第三位小数={third_decimal}, 目标阈值={threshold:.6}");
        threshold
    };
    ...
    let should_skip_calculation = {
        ...
        let min_rks_in_b27 = if b27_full {
            precalculated.b27_records.last().map_or(0.0, |r| r.rks)
        } else {
            0.0
        };
        let min_rks_in_ap3 = if ap3_full {
            precalculated
                .ap_records_sorted_by_rks
                .get(2)
                .map_or(0.0, |r| r.rks)
        ...
        if b27_full
            && ap3_full
            && potential_ap_rks < min_rks_in_b27
            && potential_ap_rks < min_rks_in_ap3
        {
            log::debug!("预过滤: 目标谱面 {target_chart_id_full} (定数 {target_chart_constant:.1}) 低于 B27 最低 RKS ({min_rks_in_b27:.4}) 和 AP3 最低 RKS ({min_rks_in_ap3:.4})，跳过推分计算");
            true // 跳过计算
        } else {
            false // 不跳过
        }
    ...
    if current_exact_rks >= target_rks_threshold {
        log::debug!("推分计算: 玩家 RKS ({current_exact_rks:.6}) 已达到或超过目标阈值 ({target_rks_threshold:.6})，无需推分 {target_chart_id_full}");
    ...
    if simulated_rks_at_100 < target_rks_threshold {
        log::debug!("推分计算: {target_chart_id_full} ACC 100% (模拟RKS {simulated_rks_at_100:.6}) 仍无法达到目标RKS阈值 ({target_rks_threshold:.6})");
    ...
    let mut high = 100.0;
    log::debug!("开始二分查找推分ACC for {target_chart_id_full}, 区间: [{low:.4}, {high:.4}]");
    ...
    for i in 0..100 {
        ...
        if simulated_rks >= target_rks_threshold {
            ...
            log::trace!("迭代#{i}: mid={mid:.4} (模拟RKS {simulated_rks:.6}) 满足条件，新区间[{low:.4}, {high:.4}]");
        } else {
            low = mid; // mid 不满足条件，需要更高的 acc
            log::trace!("迭代#{i}: mid={mid:.4} (模拟RKS {simulated_rks:.6}) 不满足条件，新区间[{low:.4}, {high:.4}]");
        }
    }

    log::debug!("二分查找结束 for {target_chart_id_full}, 结果 high = {high:.6}");
