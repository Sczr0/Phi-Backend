...
pub fn ensure_covers_available() -> AppResult<()> {
    ...
    if !covers_path.exists() {
        println!("本地曲绘目录 '{COVERS_DIR}' 不存在，尝试创建并克隆...");
    ...
    match fs::read_dir(covers_path) {
        Ok(entries) => {
            ...
            if is_empty {
                println!("本地曲绘目录 '{COVERS_DIR}' 为空或只包含隐藏文件，尝试克隆...");
                // 清理可能存在的旧的克隆失败残留
                if Path::new(COVERS_DIR).join(".git").exists() {
                    println!("清理旧的 .git 目录...");
                    fs::remove_dir_all(Path::new(COVERS_DIR).join(".git"))
...
pub fn load_local_cover(song_id: &str, size: (u32, u32)) -> RgbaImage {
    ...
    // 如果不是，需要调整此路径
    let path_png = PathBuf::from(COVERS_DIR)
        .join("illLow")
        .join(format!("{song_id}.png"));
    let path_jpg = PathBuf::from(COVERS_DIR)
        .join("illLow")
        .join(format!("{song_id}.jpg"));

    let img_result: Result<DynamicImage, image::ImageError> = if path_png.exists() {
        image::open(&path_png)
