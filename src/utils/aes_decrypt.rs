...
pub fn decrypt_save_data(data: &[u8]) -> AppResult<String> {
    ...
    if padding > 16 || padding == 0 {
        return Err(AppError::SaveDecryptError("无效的PKCS#7填充".to_string()));
    }
...
pub fn parse_json(json_str: &str) -> AppResult<Value> {
    serde_json::from_str(json_str)
        .map_err(|e| AppError::SaveDecryptError(format!("JSON解析失败: {e}")))
}
...
pub fn decrypt_save_data_base64(encrypted_data: &str) -> AppResult<String> {
    ...
    // 创建解密器
    let cipher = Decryptor::<Aes128>::new_from_slices(key, iv).map_err(|e| {
        AppError::SaveDecryptError(format!("创建解密器失败 (Key/IV length error): {e}"))
    })?;
    ...
    let decrypted_data =
        cipher
            ...
            .map_err(|e| match e {
                UnpadError => {
                    AppError::SaveDecryptError("AES解密失败: 无效的 PKCS#7 填充".to_string())
